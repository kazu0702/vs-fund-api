/*************************************************
 * VS-FUND API – server.js (env, routes, debug+raw)
 *************************************************/
...
for (const p of [path.resolve(__dirname, "..", "env", ".env"), path.resolve(process.cwd(), ".env")]) {
  if (fs.existsSync(p)) { require("dotenv").config({ path: p }); loadedEnvPath = p; break; }
}
...
clean(); setInterval(clean, 60 * 60 * 1000);
...
app.use(cors({ origin: ORIGIN, credentials: true, methods: ["GET","POST"], allowedHeaders: ["Content-Type"], maxAge: 86400 }));
...
app.post("/api/_debug/sendgrid", async (req, res) => {
  try {
    ...
    if (!process.env.SENDGRID_API_KEY) return res.status(500).json({ ok:false, error:"SENDGRID_API_KEY is missing" });
    const sgMail = require("@sendgrid/mail");
    sgMail.setApiKey(process.env.SENDGRID_API_KEY);
    await sgMail.send({
      to,
      from: { email: process.env.SENDGRID_FROM_EMAIL || "k-hirai@hirai-syoji.com", name: process.env.SENDGRID_FROM_NAME || "Victim Support Fund（VSファンド）" },
      replyTo: { email: process.env.SENDGRID_FROM_EMAIL || "k-hirai@hirai-syoji.com", name: process.env.SENDGRID_FROM_NAME || "Victim Support Fund（VSファンド）" },
      subject: "VSファンド デバッグ送信",
      text: "このメールが届けば SendGrid は有効です。",
...
/* ===== Memberstack 取得/更新（raw対応） ===== */
app.post("/api/_debug/memberstack", async (req, res) => {
  try {
    const { memberId, email, raw } = req.body || {};
    ...
    if (email) {
      ...
      return res.json(raw ? { ok:true, raw: m2 } : { ok:true, id: m2.id, email: m2.email });
    } else {
      const m = await ms.members.retrieve({ id: memberId });
      return res.json(raw ? { ok:true, raw: m } : { ok:true, id: m.id, email: m.email });
    }
  } catch (err) {
...
app.post("/api/change-plan", async (req, res) => {
  try{
    ...
    const sub = subs.data[0]; if (!sub) return res.status(404).json({ error:"No active subscription" });
    const updated = await stripe.subscriptions.update(sub.id,{ items:[{ id:sub.items.data[0].id, price:newPriceId }], proration_behavior:"none" });
